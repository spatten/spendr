...
Startup using Includes
* in SpendrModel
** get rid of the onCacheUpdate calls to sumExpendituresOverCategories
** get rid of the indexes in the initialization
** get rid of the indexes in the rest of the UI
** get the current_user_id in by adding it in the flashVars in flex/index.html.erb
** do a show on the current_user_id in SpendrModel initialization
** in SpendrModel#onCacheUpdate for User, set categories and expenditures
** Voila!  only one hit on the server for startup.

Oops, we're missing the category stats

* create Category#expenditure_sum and Category#expenditure_count.
* include them in the UsersController#show fxml
* this works, but doesn't update when you update or create an Expenditure
* call sumExpendituresOverCategories on Expenditure update, delete and create success.

h1. Paperclip

<pre>
script/plugin install git://github.com/thoughtbot/paperclip.git
rm -rf vendor/plugins/paperclip/.git</pre>
* add paperclip stuff to User.rb
** has_attached_file
** make paperclip stuff accessible in user (@attr_accessible@)
* add User#avatar_thumb and User#avatar_small, send them down in UsersController#show and add them to User.as
* Add file upload to UserBox.mxml
* Add UsersController#update

h1. To Do

# DONE Currency formatter in _model
# DONE stats pane
# DONE Do it again using includes.
## DONE Add the user_id in the flashVars
## DONE do a show on the User
## DONE include categories and expenditures
## DONE Sum the expenditures on the server, send it down as a method -- you still need ignore, but remove the onCacheUpdate calculation.
### DONE Not quite as good -- you don't get the magic onCacheUpdate updating of the totals!  
#### DONE One Solution: include the cateogry, and the sum, on the result when you make/update a new Expenditure.
#### DONE Better option -- do the sum, just over the current category, with the onSuccess callback from Expenditure update or create.
# file uploads
# testing XML
